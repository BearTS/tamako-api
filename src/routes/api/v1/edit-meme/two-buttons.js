const router = require('express').Router();
const { v4: uuidv4, validate } = require('uuid');
const { TwoButtons } = require('../../../../controllers/edit-meme');
const { canvasData } = require('../../../../database/main');
const { authorizeUser } = require('../../../../middleware/authorize');
const { errorResponse } = require('../../../../helper/ApiResponse');

router.get('/', authorizeUser, async (req, res) => {
    const { first, second } = req.query;

    var errors = [];
    if (!first)
        errors.push({ status: 406, details: 'No first parameter provided' });
    else if (first.length > 150)
        errors.push({ status: 406, details: 'First too long' });

    if (!second)
        errors.push({ status: 406, details: 'No second parameter provided' });
    else if (second.length > 150)
        errors.push({ status: 406, details: 'Second too long' });
    
    if (errors.length !== 0) {
        let body = errors;
        errors = [];
        return errorResponse(req, res, body, 400);
    }
    
    const id = uuidv4();
    await canvasData.push('edit-meme.TwoButtons', {
        id,
        first,
        second,
    });
    res.status(200).json({
        success: true,
        status: 200,
        link: `${req.protocol}://${req.get('host')}/api/v1/canvas/edit-meme/two-buttons/${id}`
    });
});

router.get('/:uuid', async (req, res) => {
    if (!validate(req.params.uuid))
        return;
        
    const arr = await canvasData.get('edit-meme.TwoButtons');
    const data = arr.filter(x => x.id === req.params.uuid);
    try {
        const buffer = await TwoButtons(data[0].first, data[0].second);
        if (buffer === 0) return errorResponse(req, res, 'Invalid image url', 406);
        res.writeHead(200, { 'Content-Type': 'image/jpg' });
        res.end(buffer);
    } catch (err) {
        errorResponse(req, res, err.message);
    }
});

module.exports = router;