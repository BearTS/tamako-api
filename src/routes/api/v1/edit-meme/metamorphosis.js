...
router.get('/', authorizeUser, async (req, res) => {
    ...
    const id = uuidv4();
    await canvasData.push('edit-meme.metamorphosis', {
        id,
        name,
        image,
    });
    res.status(200).json({
        success: true,
        status: 200,
        link: `${req.protocol}://${req.get('host')}/api/v1/canvas/edit-meme/metamorphosis/${id}`
    });
});
...
router.get('/:uuid', async (req, res) => {
    ...
    const arr = await canvasData.get('edit-meme.metamorphosis');
    const data = arr.filter(x => x.id === req.params.uuid);
    try {
        const buffer = await metamorphosis(data[0].name, data[0].image);
