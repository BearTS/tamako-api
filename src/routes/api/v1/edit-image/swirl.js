...
const { swirl } = require('../../../../controllers/edit-image');
...
router.get('/', authorizeUser, async (req, res) => {
    ...
    const id = uuidv4();
    await canvasData.push('edit-image.swirl', {
        id,
        degrees,
        image,
    });
    res.status(200).json({
        success: true,
        status: 200,
        link: `${req.protocol}://${req.get('host')}/api/v1/canvas/edit-image/swirl/${id}`
    });
});
...
router.get('/:uuid', async (req, res) => {
    ...
    const arr = await canvasData.get('edit-image.swirl');
    const data = arr.filter(x => x.id === req.params.uuid);

    try {
        const buffer = await swirl(data[0].degrees, data[0].image);
