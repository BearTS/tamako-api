...
router.get('/', authorizeUser, async (req, res) => {
    ...
    const id = uuidv4();
    await canvasData.push('edit-image.jeopardyQuestion', {
        id,
        text,
    });
    res.status(200).json({
        success: true,
        status: 200,
        link: `${req.protocol}://${req.get('host')}/api/v1/canvas/edit-image/jeopardyQuestion/${id}`
    });
});
...
router.get('/:uuid', async (req, res) => {
    ...
    const arr = await canvasData.get('edit-image.jeopardyQuestion');
    const data = arr.filter(x => x.id === req.params.uuid);

    try {
        const buffer = await jeopardyQuestion(data[0].text);
